...
public class OrderDao {
	...
	private static final String query_to_get_all_orders_by_shopId ="select o.c_order_id,c.*,i.*,o.c_qunatity,si.c_price,o.c_orderd_time,o.c_status from t_customers c,t_items i,t_orders o,t_shop_items si where o.c_customer_id=c.c_customer_id and i.c_item_id=o.c_item_id and o.c_shop_id=? and si.c_item_id=i.c_item_id";  
	...
	static class OrdersResultSetExtractor implements ResultSetExtractor<List<Order>>{
		...
		public List<Order> extractData(ResultSet rs) throws SQLException,
			...
			while(rs.next()){
				...
				i.setQuantity(rs.getString("c_qunatity"));
				i.setPrice(rs.getString("C_PRICE"));
				o.getItems().add(i);
				old = o;
			}
			orders.add(old);
